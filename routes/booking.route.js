...
router.get("/", (req, res) => {
  res.send("booking API 1.0");
});


//find by name
router.get("/find", (req, res) => {
  bookingService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/Get-all-data", async (req, res) => {
  let result = await bookingService.getAllbooking();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = bookingService.updatebooking(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
