...
router.get("/", (req, res) => {
  res.send("object API 1.0");
});

//find by name
router.get("/find", (req, res) => {
  objectService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/find/:id", async (req, res) => {
  ...
  try {
    const result = await objectService.findByid(req.params.id);
    if (!result) {
...
router.get("/Get-all-data", async (req, res) => {
  let result = await objectService.getAllobject();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = objectService.updateobject(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
