...
router.get("/", (req, res) => {
  res.send("editor API 1.0");
});

//find by name
router.get("/find", (req, res) => {
  editorService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/find/:id", async (req, res) => {
  ...
  try {
    const result = await editorService.findByid(req.params.id);
    if (!result) {
...
router.get("/Get-all-data", async (req, res) => {
  let result = await editorService.getAlleditor();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
  ...
  editorService.save(req.body)
    .then(function (data) {
      ...
      res.status(201).send({
        ...
        success: true,
        data: data,
        message: "fetched editor successfully",
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = editorService.updateeditor(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
