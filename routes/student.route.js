...
router.get("/", (req, res) => {
  res.send("student API 1.0");
});


//find by name
router.get("/find", (req, res) => {
  studentService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/find/:id", async (req, res) => {
  ...
  try {
    const result = await studentService.findByid(req.params.id);
    if (!result) {
...
router.get("/Get-all-data", async (req, res) => {
  let result = await studentService.getAllstudent();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
