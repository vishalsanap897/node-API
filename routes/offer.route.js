...
router.get("/", (req, res) => {
  res.send("offer API 1.0");
});


//find by name
router.get("/find", (req, res) => {
  offerService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/find/:id", async (req, res) => {
  ...
  try {
    const result = await offerService.findByid(req.params.id);
    if (!result) {
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = offerService.updateoffer(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
