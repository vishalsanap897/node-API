...
router.get("/", (req, res) => {
  res.send("random API 1.0");
});

//find by name
router.get("/find", (req, res) => {
  randomService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/find/:id", async (req, res) => {
  ...
  try {
    const result = await randomService.findByid(req.params.id);
    if (!result) {
...
router.get("/Get-all-data", async (req, res) => {
  let result = await randomService.getAllrandom();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
  ...
  randomService.save(req.body)
    .then(function (data) {
      ...
      res.status(201).send({
        ...
        success: true,
        data: data,
        message: "fetched random successfully",
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = randomService.updaterandom(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
