...
router.get("/", (req, res) => {
  res.send("categories API 1.0");
});


//find by name
router.get("/find", (req, res) => {
  categoriesService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/Get-all-data", async (req, res) => {
  let result = await categoriesService.getAllcategories();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
...
router.post("/add-data", (req, res) => {
  categoriesService.save(req.body)
    .then(function (data) {
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = categoriesService.updatecategories(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
