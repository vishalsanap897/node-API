...
router.get("/", (req, res) => {
  res.send("test API 1.0");
});

//find by name
router.get("/find", (req, res) => {
  testService.findByName(req.query.name)
    .then((data) => {
      res.status(200).send({
        status: 200,
        success: true,
...
router.get("/Get-all-data", async (req, res) => {
  let result = await testService.getAlltest();
  res.status(200).send({
    status: 200,
    success: true,
    data: result,
...
router.put("/update/:id", (req, res) => {
  console.log("Routes", req.params.id);
  try {
    const result = testService.updatetest(req.params.id,req.body);
    res.send({
      status: 204,
      data: result,
      success: true,
